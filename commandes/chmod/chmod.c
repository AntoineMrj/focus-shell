...
int chmode(int argc, char *argv[]){
  ...
  */
  ...
  switch(atoi(droits)/100%10){ //on récupère le chiffre des centaines
    ...
    case 7: //lire, ecrire et executer
      ...
      break;
    default:
      print("Erreur: droits mal formulés\n");
      break;
  }

  switch(atoi(droits)/10%10){ //on récupère le chiffre des dizaines
  ...
  switch(atoi(droits)%10){ //on récupère le chiffre des unités
