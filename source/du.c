...
int du(int argc, char *argv[])
{
  ...
  if (argc == 1 || (argv[1][0] == '.' && !argv[1][1]) || (argv[1][1] == '.' && !argv[1][2]))
  { //si on ne met pas d'argument à la commande 'du' OU si l'argument passé est le répertoire courant (.) OU le répertoire parent (..)
    if (argc == 1)
    {                                         //si pas d'argument
    ...
    {                          // si l'argument passé est le répertoire parent (..)
    ...
    while ((dptr = readdir(dirp)))
    {
      ...
      { //si c'est un dossier, on affiche sa taille
        if ((dptr->d_name[0]) != '.')
        {
          stat(dptr->d_name, &s);
      ...
      { //si c'est un fichier, on n'affiche pas sa taille, mais on l'ajoute à 'size'
        stat(dptr->d_name, &s);
        size = size + s.st_size;
      }
    }
  ...
  { //si on met un argument à la commande 'du'
    for (int i = 1; i < argc; i++)
    {
      if (!(dirp = opendir(argv[i])))
      { //si cet argument est un fichier
        filename = argv[i];
