...
void mv(int argc, char *argv[])
{
  ...
  if(argc<2){
    printErr("mv: opérande manquant\n");
  }
  else{
    destination = opendir(argv[argc-1]);

    if(destination==NULL){ //si le dernier argument n'est pas un répertoire mais un fichier
      if(argc>3){ //si on a plus de 2 noms de fichier en argument, alors on s'attend à déplacer les (n-1) premiers fichiers dans le n-ième argument qui doit être un réperoire. Or celui ci est un fichier.
        printErr("La cible '%s' n'est pas un répertoire.\n",argv[argc-1]); //Donc on renvoie une erreur.
        return;
      }
      else{ //si on a 2 noms de fichiers en argument, alors on renomme le 1er fichier avec le nom du 2ème fichier
        ...
        remove(argv[1]); //on supprime le fichier source
      }
    }
    else{ //si le dernier argument est un répertoire
      closedir(destination);
      for(int i=1; i<argc-1; i++){ //on déplace les (n-1) fichiers dans le répertoire
        ...
        char chaine[1000] = "";
        if (source == NULL) //si le fichier à déplacer n'existe pas
        {
        ...
        fclose(source);
        fclose(copieSource);
        remove(argv[i]); //on supprime le fichier source
      }
    }
  }
}
